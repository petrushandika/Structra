...
This document explains Structra API endpoints, request/response formats, and usage examples.

---

## 🔑 Authentication

Structra uses API key authentication. Include API key in header of every request:
...
Analyze visual design and generate canonical schema.
...
```json
{
  ...
  "textualDescription": "Hero section with background blob on the right",
...
Generate code from existing canonical schema.
...
Validate canonical schema before code generation.
...
Retrieve project information and history.
...
curl -X POST https://api.structra.com/v1/analyze \
  ...
  -d '{
    ...
    "textualDescription": "Hero section with background blob",
...
Structra can send webhook events for async notifications.
...
6. **Handle errors** with proper error handling
