...
This document covers all shape types and patterns that Structra can generate, including flower, triangle, starburst, polygon, ribbon, blob, wave, curve, custom corners, borders, section dividers, tooltips, and loaders.
...
### Use CSS-only (Mask/Clip-path) when:
- Shape is geometric or can be achieved with CSS
- Performance is critical
- Shape doesn't need complex paths
- Browser support allows

### Use SVG when:
- Shape is too complex for CSS
- Need precise path control
- Shape needs path animations
- Complex gradients or patterns required
